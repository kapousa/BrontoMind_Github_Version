...
def get_model_id():
    model_id = numpy.array(ModelProfile.query.with_entities(ModelProfile.model_id).first())
    flatten_model_id = model_id.flatten()
    return flatten_model_id[0]


def get_features():
    model_features = numpy.array(ModelFeatures.query.with_entities(ModelFeatures.feature_name).all())
    flatten_model_features = model_features.flatten()
    return flatten_model_features


def get_labels():
    model_labels = numpy.array(ModelLabels.query.with_entities(ModelLabels.label_name).all())
...
def get_encoded_columns(column_type):
    model_encoded_columns = numpy.array(
        ModelEncodedColumns.query.with_entities(ModelEncodedColumns.column_name).filter_by(
            column_type=column_type).all())
    flatten_model_encoded_column = model_encoded_columns.flatten()
    return flatten_model_encoded_column


def get_encoded_labels():
    model_encoded_columns = numpy.array(ModelEncodedColumns.query.with_entities(ModelEncodedColumns.column_name).all())
    flatten_model_encoded_column = model_encoded_columns.flatten()
    return flatten_model_encoded_column


def get_model_name():
    model_name = numpy.array(ModelProfile.query.with_entities(ModelProfile.model_name).first())
...
def delete_encoded_columns():
    aa = ModelEncodedColumns.query.filter().delete()
    db.session.commit()
    return 1
